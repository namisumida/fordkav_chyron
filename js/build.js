...
// Y positions
var pos_chyron = 160;
var pos_brandtext = 150;
var pos_gridstart = 135;
var pos_chyronback = 180;
...
// DEFINING CONVERSION FUNCTIONS
	...
	// BUILD SCREENS
	var dataset;
	var screen1 = svg.append("g");
	var screen2 = svg.append("g");
	...
	d3.csv("Data/KavanaughFord_longdata.csv", rowConverter, function(data) {
		...
		screen1.append("text")
		   .attr("class", "intro_text")
			 .attr("id", "intro1")
			 .text("You know those captions that appear on the lower part of your TV screen when you’re watching \
			 cable news? They look something like this:")
			 .attr("x", 10)
			 .attr("y", 12)
			 .call(wrap, w_svg-20);
		...
		// More text
		screen1.append("text")
				   .text("These captions, also known as “lower-third chyrons,” are oftentimes treated like headlines of an article -\
								 quick summaries of the news and topic at-hand. And while many of these captions are unbiased overviews of\
						 		 the news topic, they can still be a window into how breaking news stories are reported by cable news networks.")
					 .attr("class", "intro_text")
					 .attr("id", "intro1_2")
					 .attr("x", 10)
					 .attr("y", 300)
		...
		// 2: Kavanaugh Ford intro
		// Build rectangles for chyrons

		// Intro text
		screen2.append("text")
		   		 .attr("class", "intro_text")
					 .text("On September 16, 2018, MSNBC, CNN and Fox News covered the nearly six hours of Christine Blasey Ford’s\
										and Brett Kavanaugh’s hearings. While all three provided similar live video coverage, the captions that came\
										alongside them differed in who was quoted, what was emphasized, and what was not emphasized.")
					 .attr("x", 10)
					 .attr("y", 12)
					 .call(wrap, w_svg-20);
		// Move out for now
		...
		screen2.selectAll("chyron_rects")
			 ...
			 .attr("x", function(d) {
				 if (d.network=="msnbc") {
					 ...
					 return w_chyron+btwn_spacing*2;
				 }
				 else { return (w_chyron*2 + btwn_spacing*3); }
			 })
			 .attr("width", w_chyron)
		...
		screen2.selectAll("brand_text")
				...
				.attr("x", function(d) {
					if (d.network=="msnbc") {
						...
						return btwn_spacing*2+(w_chyron/2)*3;
					}
					else { return btwn_spacing*3+(w_chyron/2)*5; }
				})
		...
		screen3.selectAll("grid_rect")
					 ...
					 .attr("x", function(d,i) {
						 if (d.network=="msnbc") {
							 return btwn_spacing+(i % 10 * sq_spacing);
						 }
						 else if (d.network=="cnn") {
							 return btwn_spacing*2+(10*sq_spacing)+((i-103) % 10 * sq_spacing);
						 }
						 else { return btwn_spacing*3+(20*sq_spacing) + ((i-173) % 10 * sq_spacing); };
					 })
					 .attr("y", -500)
					 .attr("width", sq_size)
					 .attr("height", sq_size);
		...
		screen3.selectAll("chyron_back")
					 ...
					 .attr("x", function(d) {
						 if (d.network=="msnbc") {
							 return btwn_spacing+sq_spacing*4;
						 }
						 else if (d.network=="cnn") {
							 return (btwn_spacing*2)+(sq_spacing*14);
						 }
						 else { return (btwn_spacing*3)+(sq_spacing*24); }
					 })
					 .attr("y", -500)
		...
		screen3.selectAll("count_text")
					 ...
					 .attr("x", function(d) {
						 if (d.network=="msnbc") {
							 return btwn_spacing+sq_spacing*5-(sq_padding/2);
						 }
						 else if (d.network=="cnn") {
							 return (btwn_spacing*2)+(sq_spacing*15)-(sq_padding/2);
						 }
						 else { return (btwn_spacing*3)+(sq_spacing*25)-(sq_padding/2); }
					 })
					 .attr("y", -500);
		...
		screen4.append("text")
						...
						.attr("x", btwn_spacing*3+w_chyron*3+20)
						.style("font-size", 11)
						.call(wrap, 70)
						.style("fill", "none");
		screen4.append("line")
						.attr("id", "ford_line")
						.attr("class", "description")
						.attr("x1", btwn_spacing*3+w_chyron*3-5)
						.attr("x2", btwn_spacing*3+w_chyron*3+15)
		...
		screen7.append("text")
					 ...
					 .attr("x", 10)
					 .attr("y", 12)
					 .text("About a third of MSNBC's and CNN's cpations occurred during Kavanaugh's hearing, compared with 62% of Fox News' cpations.")
					 .call(wrap, w_svg-20);
		screen7.append("text")
					 .attr("class", "intro_text")
					 .attr("id", "intro7_2")
		...
		screen10.append("text")
					 ...
					 the main takeaways as deemed by the network. What is emphasized and what is not emphasized are important to notice across the three newtorks. In many \
		...
		screen11.selectAll("grid_circle")
					  .data(dataset)
						...
						.attr("cx", function(d,i) {
							if (d.network=="msnbc") {
								 return btwn_spacing+(i % 10 * sq_spacing) + sq_spacing/2;
							 }
							 else if (d.network=="cnn") {
								 return btwn_spacing*2+(10*sq_spacing)+((i-103) % 10 * sq_spacing) + sq_spacing/2;
							 }
							 else { return btwn_spacing*3+(20*sq_spacing) + ((i-173) % 10 * sq_spacing) + sq_spacing/2; };
						})
						.attr("cy", -500)
						.attr("r", sq_size-8)
						.style("fill", function(d) {
		...
		screen11.append("line")
						.attr("id", "ford_line")
						.attr("x1", btwn_spacing*3+w_chyron*3-2)
						.attr("x2", btwn_spacing*3+w_chyron*3+15)
